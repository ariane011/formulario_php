...
$objFcs = new Funcoes();
if(isset($POST['btCadastrar'])){
    if($objFcn->queryInsert($POST) == 'ok'){
        header('location: /formulario');
    }else{
        echo '<script type="text/javascript">alert("Erro em cadastrar");</script>';
    }
}
if(isset($POST['btAlterar'])){
    if($objFcn->queryUpdate($POST) == 'ok'){
        header('location: ?acao=edit&func='.$objFcs->base64($POST['func'],1));
    }else{
...
if(isset($GET['acao'])){
    switch($GET['acao']){
        case 'edit': $func = $objFcn->querySeleciona($GET['func']); break;
        case 'delet':
            if($objFcn->queryDelete($GET['func']) == 'ok'){
                header('location: /formulario');
            }else{
...
<div id="formulario">
    <form name="formCad" action="" method="post">
        ...
        <?php if(isset($GET['acao']) <> 'edit'){ ?>
        <label>Senha: </label><br>
        <input type="password" name="senha" required="required"><br>
        <?php } ?>
        <br>
        <input type="submit" name="<?=(isset($GET['acao']) == 'edit')?('btAlterar'):('btCadastrar')?>" value="<?=(isset($GET['acao']) == 'edit')?('Alterar'):('Cadastrar')?>">
